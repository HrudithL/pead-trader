"""
Finalize Strategy 5: size + FF12-sector neutralized, trim=0.15, tilt_power=1.0,
base_unit_fraction=0.0015, quarterly trailing-NAV cadence (the "max Sharpe / min drawdown" pick
from the retune in 22_sector_neutral_and_cadence.py: ann.ret 4.06%, Sharpe 1.17, max_dd -10.8%,
the best risk-adjusted profile and shallowest drawdown of any strategy built so far).

Produces the same output shape as strategy 4 (17/20) so it drops into the same report/charting
pipeline: data/positions_strategy5_v2.parquet, data/backtest_v2_strategy5.csv,
data/backtest_v2_strategy5_quarterlog.csv, data/backtest_v2_strategy5_summary.json, and an updated
data/backtest_v2_comparison.csv with all five strategies.
"""
import sys
...
DEFAULT_COST_BPS = 15
TRIM, TILT_POWER, BUF = 0.15, 1.0, 0.0015
...
pos_out.to_parquet(DATA / "positions_strategy5_v2.parquet", index=False)
...
out.to_csv(DATA / "backtest_v2_strategy5.csv", index=False)
pd.DataFrame(quarter_log).to_csv(DATA / "backtest_v2_strategy5_quarterlog.csv", index=False)
...
summary = dict(
    name="strategy5_size_sector_neutral", trim=TRIM, tilt_power=TILT_POWER,
...
with open(DATA / "backtest_v2_strategy5_summary.json", "w") as f:
    json.dump(summary, f, indent=2)
print(f"strategy5: ann.ret={ann_ret:.2%} ann.vol={ann_vol:.2%} Sharpe={sharpe:.3f} "
      f"maxDD={max_dd:.2%} final_nav=${nav[-1]:,.0f}")

# ---------- update the comparison table to include all 5 strategies ----------
comparison_rows = []
for name in ["extreme", "rankweighted", "balanced", "strategy4_tilted"]:
    with open(DATA / f"backtest_v2_{name if name != 'strategy4_tilted' else 'strategy4'}_summary.json") as f:
        comparison_rows.append(json.load(f))
comparison_rows.append(summary)
pd.DataFrame(comparison_rows).to_csv(DATA / "backtest_v2_comparison.csv", index=False)
print("\nwrote data/backtest_v2_comparison.csv (5 strategies)")
